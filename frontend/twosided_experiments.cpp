...
int main(int argc, char* argv[]) {
   ...
   if (FLAGS_storage_node) {
      ...
      std::cout << "building tree " << part.first << " to " << part.second << std::endl;
      for (uint64_t t_i = 0; t_i < FLAGS_worker; ++t_i) {
         comp.getWorkerPool().scheduleJobAsync(t_i, [&, t_i]() {
            ...
            for (Key k = begin; k < end; ++k) {
               ...
               Value v = k;
               auto success = threads::twosided::Worker::my().insert(p_id, k, v);
               ensure(success);
         ...
         comp.getWorkerPool().scheduleJobAsync(t_i, [&, t_i]() {
            ...
            for (; keep_running; threads::twosided::Worker::my().counters.incr(profiling::WorkerCounters::tx_p)) {
               ...
               if (FLAGS_scans) {
                  ...
                  auto kv_span = threads::twosided::Worker::my().scan(p_idx, start, start + expected_values);
                  if (kv_span.empty())
                     throw std::logic_error("empty span " + std::to_string(start) + " expected " +
                                            std::to_string(expected_values) + " in partition " + std::to_string(p_idx));
