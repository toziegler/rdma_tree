...
struct __attribute__((packed)) PageHeader {
   ...
   uint8_t buffer[BUFFER_SIZE];  // can be used to piggy back additional information to keep size small
};
...
struct MetadataPage : public PageHeader {
   ...
   RemotePtr getRootPtr() { return (*reinterpret_cast<RemotePtr*>(&buffer[0])); }
   void setRootPtr(RemotePtr remote_ptr) { *reinterpret_cast<RemotePtr*>(&buffer[0]) = remote_ptr; }
};
static_assert(sizeof(PageHeader) <= 32, "PageHeaer larger than 32 Byte");
