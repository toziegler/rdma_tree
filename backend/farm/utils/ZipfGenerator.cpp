...
{
// -------------------------------------------------------------------------------------
ZipfGenerator::ZipfGenerator(double ex_n, double theta) : n(ex_n - 1), theta(theta)
{
   alpha = 1.0 / (1.0 - theta);
   zetan = zeta(n, theta);
...
// -------------------------------------------------------------------------------------
double ZipfGenerator::zeta(double n, double theta)
{
   double ans = 0;
   for (u64 i = 1; i <= static_cast<u64>(n); i++)
...
uint64_t ZipfGenerator::rand()
{
   ...
   if (uz < (1 + std::pow(0.5, theta)))
      return 2;
   u64 ret = 1 + static_cast<uint64_t>(n * pow(eta * u - eta + 1, alpha));
   return ret;
}
// -------------------------------------------------------------------------------------
}  // namespace utils
