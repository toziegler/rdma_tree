...
def compile(servers):
    servers.cd("/home/tziegler/farm/build")
...
def ycsbBenchmark(servers, dramGB, numberNodes, zipf, readRatio, initialFillDegree ,pp,fp, partitioned):
    servers.cd("/home/tziegler/farm/build/frontend")
    ...
    for i in range(0, numberNodes):
        cmd = f'numactl --membind=0 --cpunodebind=0 ./ycsb -worker=20 -dramGB={dramGB} -nodes={numberNodes} -messageHandlerThreads=4   -ownIp={servers[i].ibIp} -pageProviderThreads={pp} -coolingPercentage=10 -freePercentage={fp} -csvFile=ycsb_scale_server_fixed_data.csv -YCSB_read_ratio={readRatio} -YCSB_run_for_seconds=30 -YCSB_tuple_count={numTuples} -YCSB_zipf_factor={zipf} -{partitioned} -tag={partitioned} -evictCoolestEpochs=0.5'
