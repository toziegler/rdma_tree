...
def ycsbBenchmark(servers, dramGB, numberNodes ,pp,fp):
    ...
    for i in range(0, numberNodes):
        cmd = f'numactl --membind=0 --cpunodebind=0 ./pp_test_scan_no_remote_page_effect -worker=20 -dramGB={dramGB} -nodes={numberNodes} -messageHandlerThreads=4   -ownIp={servers[i].ibIp} -pageProviderThreads={pp} -coolingPercentage=10 -freePercentage={fp} -csvFile=pp_test_scan_no_remote_page_effect.csv'
